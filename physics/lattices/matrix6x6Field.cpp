...
physics::lattices::Matrix6x6Field::Matrix6x6Field(const hardware::System& system,
                                                  const GaugefieldParametersInterface* parameters)
    : system(system), latticeObjectParameters(parameters), matrix6x6Field(system)
{
}

void physics::lattices::Matrix6x6Field::setField(const physics::lattices::Gaugefield* gaugefield,
                                                 const bool upperOrLower)
{
    if (latticeObjectParameters->getFermact() != common::action::clover) {
        throw Invalid_Parameters("The setField method in matrix6x6Field is not to be used with wilson or tm fermion "
                                 "action!",
                                 "clover", latticeObjectParameters->getFermact());
    }
    ...
    if (num_devs == 1) {
        ...
        upperOrLower
            ? device->getMatrix6x6FieldCode()
                  ->clover_eo_inverse_explicit_upper_left_device(matrix6x6FieldBuffers[0], gaugefieldBuffers[0],
                                                                 latticeObjectParameters->getKappa(),
                                                                 latticeObjectParameters->getCsw())
            : device->getMatrix6x6FieldCode()
                  ->clover_eo_inverse_explicit_lower_right_device(matrix6x6FieldBuffers[0], gaugefieldBuffers[0],
                                                                  latticeObjectParameters->getKappa(),
                                                                  latticeObjectParameters->getCsw());
    } else {
        for (size_t i = 0; i < num_devs; ++i) {
            ...
            upperOrLower
                ? device->getMatrix6x6FieldCode()
                      ->clover_eo_inverse_explicit_upper_left_device(matrix6x6FieldBuffers[i], gaugefieldBuffers[i],
                                                                     latticeObjectParameters->getKappa(),
                                                                     latticeObjectParameters->getCsw())
                : device->getMatrix6x6FieldCode()
                      ->clover_eo_inverse_explicit_lower_right_device(matrix6x6FieldBuffers[i], gaugefieldBuffers[i],
                                                                      latticeObjectParameters->getKappa(),
                                                                      latticeObjectParameters->getCsw());
        }
    }
...
void physics::lattices::S_det(const Scalar<hmc_float>* res, const Gaugefield& field, const hmc_float kappa,
                              const hmc_float csw)
{
    auto field_buffers = field.get_buffers();
