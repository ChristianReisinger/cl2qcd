 ...
 * Copyright (c) 2014-2017 Francesca Cuteri
...
void physics::lattices::scalar_product_real_part(const Scalar<hmc_float>* res, const Spinorfield_eo& left,
                                                 const Spinorfield_eo& right)
{
    auto res_buffers   = res->get_buffers();
    auto left_buffers  = left.get_buffers();
...
void physics::lattices::saxpy(const Spinorfield_eo* out, const hmc_float alpha, const Spinorfield_eo& x,
                              const Spinorfield_eo& y)
{
    auto out_bufs = out->get_buffers();
    auto x_bufs   = x.get_buffers();
...
void physics::lattices::saxpy(const Spinorfield_eo* out, const Vector<hmc_float>& alpha, const int index_alpha,
                              const Spinorfield_eo& x, const Spinorfield_eo& y)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
...
void physics::lattices::saxpy(const Spinorfield_eo* out, const Scalar<hmc_float>& alpha, const Spinorfield_eo& x,
                              const Spinorfield_eo& y)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
...
void physics::lattices::saxpby(const Spinorfield_eo* out, const hmc_complex alpha, const Spinorfield_eo& x,
                               const hmc_complex beta, const Spinorfield_eo& y)
{
    auto out_bufs = out->get_buffers();
    auto x_bufs   = x.get_buffers();
    auto y_bufs   = y.get_buffers();
...
void physics::lattices::saxpby(const Spinorfield_eo* out, const Vector<hmc_float>& alpha, const int index_alpha,
                               const Spinorfield_eo& x, const Vector<hmc_float>& beta, const int index_beta,
                               const Spinorfield_eo& y)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
    ...
    for (size_t i = 0; i < out_bufs.size(); ++i) {
        auto out_buf = out_bufs[i];
        auto device  = out_buf->get_device();
        device->getSpinorCode()->saxpby_eoprec_device(x_bufs[i], y_bufs[i], alpha_bufs[i], beta_bufs[i], index_alpha,
                                                      index_beta, out_buf);
    }
...
void physics::lattices::sax(const Spinorfield_eo* out, const Vector<hmc_float>& alpha, const int index_alpha,
                            const Spinorfield_eo& x)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
    auto x_bufs     = x.get_buffers();
...
template<>
size_t physics::lattices::get_flops<physics::lattices::Spinorfield_eo, hmc_complex, physics::lattices::saxpy>(
    const hardware::System& system)
{
    // assert single system
    auto devices     = system.get_devices();
