                                              ...
                                              const physics::AdditionalParameters& additionalParameters)
{
    ...
    if (interfacesHandler.getMetropolisParametersInterface().getFermact() ==
        common::action::clover) {  // for details see hep-lat/9603008
           ...
           physics::InterfacesHandler& interfacesHandler)
{
    ...
    if (!parametersInterface.getUseGaugeOnly()) {
        if (parametersInterface.getUseMp()) {
            if (parametersInterface.getFermact() == common::action::rooted_stagg ||
                parametersInterface.getFermact() == common::action::clover) {
                throw Invalid_Parameters("Mass preconditioning not implemented for staggered or clover fermions!",
                                         "NOT rooted_stagg or clover", parametersInterface.getFermact());
            }
            ...
            if (interfacesHandler.getMetropolisParametersInterface().getFermact() == common::action::clover) {
                spinor_energy_init_updated = spinor_energy_init +
                                             physics::lattices::S_det(gf,
                                                                      interfacesHandler
                                                                          .getAdditionalParameters<SPINORFIELD>(false)
                                                                          .getKappa(),
                                                                      interfacesHandler
                                                                          .getAdditionalParameters<SPINORFIELD>(false)
                                                                          .getCsw());
            ...
            // check on NANs
            if (spinor_energy_init_updated != spinor_energy_init_updated || s_fermion_final != s_fermion_final ||
                deltaH != deltaH) {
                throw Print_Error_Message("NAN occured in Metropolis! Aborting!", __FILE__, __LINE__);
            }
        }
