...
static hmc_float calc_s_fermion(const physics::lattices::Gaugefield& gf, const ROOTED_SPINORFIELD& phi,
...
template<class ROOTED_SPINORFIELD, class SPINORFIELD, class FERMIONMATRIX>
static hmc_float
calc_s_fermion(const physics::lattices::Gaugefield& gf, const ROOTED_SPINORFIELD& phi, const hardware::System& system,
               physics::InterfacesHandler& interfacesHandler, const physics::AdditionalParameters& additionalParameters)
{
    ...
    for (const auto& phi_j : phi) {
        ...
        // Wilson:           (QplusQminus)^{-\frac{N_f}{2}}\,\phi_j
        // Staggered:        (MdagM)^{-\frac{N_f}{4}}\,\phi_j
        SPINORFIELD tmp(system, interfacesHandler.getInterface<SPINORFIELD>());
        sax(&tmp, {phi.get_a0(), 0.}, *phi_j);
        for (unsigned int i = 0; i < phi.getOrder(); i++) {
            saxpy(&tmp, (phi.get_a())[i], *X[i], tmp);
        }
        logger.trace() << "Calulated S_fermion, solver took " << iterations << " iterations.";
...
hmc_float physics::algorithms::calc_s_fermion(const physics::lattices::Gaugefield& gf,
                                              const physics::lattices::wilson::Rooted_Spinorfield& phi,
                                              ...
                                              const physics::AdditionalParameters& additionalParameters)
{
    ...
    return ::calc_s_fermion<Rooted_Spinorfield, Spinorfield, QplusQminus>(gf, phi, system, interfacesHandler,
                                                                          additionalParameters);
}

hmc_float physics::algorithms::calc_s_fermion(const physics::lattices::Gaugefield& gf,
                                              const physics::lattices::wilson::Rooted_Spinorfield_eo& phi,
                                              const hardware::System& system,
                                              physics::InterfacesHandler& interfacesHandler,
                                              const physics::AdditionalParameters& additionalParameters)
{
    ...
    return ::calc_s_fermion<Rooted_Spinorfield_eo, Spinorfield_eo, QplusQminus_eo>(gf, phi, system, interfacesHandler,
                                                                                   additionalParameters);
}
...
/**
 * Removing the function calc_s_fermion_mp for either Rooted_Staggeredfield_eo or Rooted_Spinorfield_eo or
 * Rooted_Spinorfield would break code compilation
 */
...
hmc_float physics::algorithms::calc_s_fermion_mp(const physics::lattices::Gaugefield&,
                                                 const physics::lattices::wilson::Rooted_Spinorfield_eo&,
                                                 const hardware::System&, physics::InterfacesHandler&)
{
    throw std::runtime_error("Not implemented!");
}
...
hmc_float physics::algorithms::calc_s_fermion_mp(const physics::lattices::Gaugefield&,
                                                 const physics::lattices::wilson::Rooted_Spinorfield&,
                                                 const hardware::System&, physics::InterfacesHandler&)
{
    throw std::runtime_error("Not implemented!");
}
                                              ...
                                              const physics::AdditionalParameters& additionalParameters)
{
    ...
    return ::calc_s_fermion<Rooted_Staggeredfield_eo, Staggeredfield_eo, MdagM_eo>(gf, phi, system, interfacesHandler,
                                                                                   additionalParameters);
}
           ...
           physics::InterfacesHandler& interfacesHandler)
{
    ...
    if (!parametersInterface.getUseGaugeOnly()) {
        if (parametersInterface.getUseMp()) {
            ...
            if (parametersInterface.getFermact() == common::action::rooted_wils) {
                throw Invalid_Parameters("Mass preconditioning not implemented for rooted wilson fermions!",
                                         "NOT rooted_wils", parametersInterface.getFermact());
            }
...
physics::algorithms::metropolis(const hmc_float rnd, const hmc_float beta, const physics::lattices::Gaugefield& gf,
                                ...
                                const physics::lattices::wilson::Rooted_Spinorfield_eo* const phi_mp,
                                const hmc_float spinor_energy_mp_init, const hardware::System& system,
                                physics::InterfacesHandler& interfacesHandler)
{
    return ::metropolis(rnd, beta, gf, new_u, p, new_p, phi, spinor_energy_init, phi_mp, spinor_energy_mp_init, system,
...
physics::algorithms::metropolis(const hmc_float rnd, const hmc_float beta, const physics::lattices::Gaugefield& gf,
                                ...
                                const physics::lattices::wilson::Rooted_Spinorfield& phi,
                                const hmc_float spinor_energy_init,
                                const physics::lattices::wilson::Rooted_Spinorfield* const phi_mp,
                                const hmc_float spinor_energy_mp_init, const hardware::System& system,
                                physics::InterfacesHandler& interfacesHandler)
{
    return ::metropolis(rnd, beta, gf, new_u, p, new_p, phi, spinor_energy_init, phi_mp, spinor_energy_mp_init, system,
