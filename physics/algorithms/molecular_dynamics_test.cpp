...
{
    using namespace physics::lattices;
    const char* _params[] = {"foo", "--nTime=4", "--fermionAction=wilson", "--nDevices=1"};
    meta::Inputparameters params(4, _params);
    physics::InterfacesHandlerImplementation interfacesHandler{params};
    hardware::HardwareParametersImplementation hP(&params);
    hardware::code::OpenClKernelParametersImplementation kP(params);
    ...
    physics::algorithms::find_maxmin_eigenvalue(maxEigen, minEigen, Qpm, gf, system, interfacesHandler,
                                                parametersInterface.getFindMinMaxPrec(),
                                                interfacesHandler
                                                    .getAdditionalParameters<wilson::Rooted_Spinorfield>());
    ...
    pseudo_randomize<Spinorfield, spinor>(in[0].get(), 13);
    pseudo_randomize<Spinorfield, spinor>(out[0].get(), 14);
    physics::algorithms::md_update_spinorfield(&out, gf, in, system, interfacesHandler,
                                               interfacesHandler.getAdditionalParameters<wilson::Rooted_Spinorfield>());

    BOOST_CHECK_CLOSE(squarenorm(*out[0].get()), 2588.2852324465166, 1e-8);
}
...
BOOST_AUTO_TEST_CASE(md_update_gaugemomentum_detratio)
{
    {
        using namespace physics::lattices;
        const char* _params[] = {"foo", "--nTime=4", "--kappaMP=.25", "--nDevices=1"};
        meta::Inputparameters params(4, _params);
        physics::InterfacesHandlerImplementation interfacesHandler{params};
        hardware::HardwareParametersImplementation hP(&params);
        hardware::code::OpenClKernelParametersImplementation kP(params);
