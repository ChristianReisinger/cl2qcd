                                             ...
                                             const physics::AdditionalParameters& additionalParameters)
{
    ...
    for (unsigned int i = 0; i < phi.getOrder(); i++) {
        X_e.emplace_back(
            std::make_shared<Spinorfield_eo>(system,
    ...
    cg_m(X_e, fm, gf, phi.get_b(), *(phi[0]), system, interfacesHandler, parametersInterface.getSolverForcePrecision(),
         additionalParameters);
    ...
    if (parametersInterface.getFermact() == common::action::wilson) {
        for (unsigned int i = 0; i < phi.getOrder(); i++) {
            ...
            logger.debug() << "X_o"
                           << "[" << i << "]: squarenorm: " << squarenorm(*X_o[i]);
            logger.debug() << "Y_e"
                           << "[" << i << "]: squarenorm: " << squarenorm(*Y_e[i]);
                                             ...
                                             const physics::AdditionalParameters& additionalParameters)
{
    ...
    for (unsigned int i = 0; i < phi.getOrder(); i++) {
        X.emplace_back(
            std::make_shared<Spinorfield>(system, interfacesHandler.getInterface<physics::lattices::Spinorfield>()));
        Y.emplace_back(
            std::make_shared<Spinorfield>(system, interfacesHandler.getInterface<physics::lattices::Spinorfield>()));
    }
    ...
    const QplusQminus fm(system, interfacesHandler.getInterface<physics::fermionmatrix::QplusQminus>());
    //*(phi[0]) in the following line is to use the only pseudofermion that can be used in Wilson rhmc
    cg_m(X, fm, gf, phi.get_b(), *(phi[0]), system, interfacesHandler, parametersInterface.getSolverForcePrecision(),
         additionalParameters);
    ...
    for (unsigned int i = 0; i < phi.getOrder(); i++) {
        ...
        physics::lattices::saxpy(force, (phi.get_a())[i],
                                 tmp);  // I removed the -1* because I think in the Wilson case the "-" is already
                                        // contained in the fermion force calculation.
    }
}
