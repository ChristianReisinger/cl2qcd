...
static int getRationalApproximationDenominator(std::string whichRationalApproximation, int numTastesDecimalDigits,
                                               common::action action, int numPseudoFermions);
    ...
    if (parameters.get_read_rational_approximations_from_file()) {
        ...
        if (parameters.get_fermact() == common::action::wilson) {
            if (approx_hb->Get_order() != parameters.get_metro_approx_ord() ||
                ...
                approx_hb->Get_exponent() * 4 * parameters.get_num_pseudofermions() != parameters.get_num_tastes() ||
                approx_md->Get_exponent() * 2 * (-1) * parameters.get_num_pseudofermions() !=
                    parameters.get_num_tastes() ||
                approx_met->Get_exponent() * 2 * (-1) * parameters.get_num_pseudofermions() !=
                    parameters.get_num_tastes() ||
                ...
                throw Print_Error_Message("The parameters of at least one Rational Approximation read from file are "
                                          "not coherent with those given as input!");
            }
            ...
            if (approx_hb->Get_order() != parameters.get_metro_approx_ord() ||
                ...
                approx_md->Get_exponent() * 4 * (-1) * parameters.get_num_pseudofermions() !=
                    parameters.get_num_tastes() ||
...
void rhmcExecutable::checkRhmcParameters(const meta::Inputparameters& p)
{
    if ((p.get_fermact() != common::action::rooted_stagg) &&
        (p.get_fermact() != common::action::wilson))  // add && commom::action::wilson
        throw Invalid_Parameters("Fermion action not suitable for RHMC!", "rooted_stagg or wilson", p.get_fermact());
    if (!p.get_use_eo() &&
        (p.get_fermact() == common::action::rooted_stagg))  // distinguish between wilson and staggered
    ...
    // TODO: Check if mass preconditioning can be used with the Wilson Rhmc. If yes, then do a distinction between
    // staggerend and wilson here.
    if (p.get_use_mp())
    ...
    if ((p.get_fermact() == common::action::rooted_stagg) &&
        (int)(p.get_num_tastes() * std::pow(10, p.get_num_tastes_decimal_digits())) %
                (4 * (int)(std::pow(10, p.get_num_tastes_decimal_digits()))) ==
            0)  // distinguish between wilson and staggered
        throw Invalid_Parameters("Staggered RHMC not working with multiple of 4 tastes (there is no need of the "
                                 "rooting trick)!",
                                 "num_tastes%4 !=0", "num_tastes=" + std::to_string(p.get_num_tastes()));
    if ((p.get_fermact() == common::action::wilson) &&
        (int)(p.get_num_tastes() * std::pow(10, p.get_num_tastes_decimal_digits())) %
                (2 * (int)(std::pow(10, p.get_num_tastes_decimal_digits()))) ==
            0)  // distinguish between wilson and staggered
        throw Invalid_Parameters("Wilson RHMC not working with multiple of 2 tastes (there is no need of the rooting "
                                 "trick)!",
                                 "num_tastes%2 !=0", "num_tastes=" + std::to_string(p.get_num_tastes()));
...
static int getRationalApproximationNumerator(double numTastes, int numTastesDecimalDigits)
{
    ...
    if ((tmpNumerator - intpart) != 0.0) {
        logger.error() << "Found option num_tastes=" << numTastes
                       ...
                       << " and this will imply a loss of precision!";
                                               ...
                                               common::action action, int numPseudoFermions)
{
    if (action == common::action::wilson) {
        if (whichRationalApproximation == "HB")
            ...
            return 2 * (int)(std::pow(10, numTastesDecimalDigits)) * numPseudoFermions;
        else
        ...
        throw Invalid_Parameters("Rational approximation denominator currently only available for wilson and "
                                 "rooted_stagg actions.",
                                 "rooted_stagg or wilson", action);
}
