...
std::vector<const hardware::buffers::matrix6x6*> hardware::lattices::Matrix6x6Field::allocate_buffers()
{
    ...
    for (auto device : devices) {
        buffers.push_back(new matrix6x6(device->getLocalLatticeMemoryExtents().getLatticeVolume(),
...
void hardware::lattices::Matrix6x6Field::send_matrix6x6_to_buffers(const Matrix6x6* const gf_host)
{
    ...
    for (auto const buffer : buffers) {
        ...
        TemporalParallelizationHandlerLink tmp2(device->getGridPos(), device->getLocalLatticeExtents(),
                                                sizeof(Matrix6x6), device->getHaloExtent());

        if (buffers.size() == 1)
            device->getMatrix6x6FieldCode()->importMatrix6x6Field(buffer, gf_host);
        else {
...
void hardware::lattices::Matrix6x6Field::update_halo_aos(const std::vector<const hardware::buffers::matrix6x6*> buffers,
...
void hardware::lattices::Matrix6x6Field::update_halo_soa(const std::vector<const hardware::buffers::matrix6x6*> buffers,
                                                         const hardware::System& system) const
{
    // check all buffers are non-soa
    for (auto const buffer : buffers) {
