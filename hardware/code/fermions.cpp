...
void hardware::code::Fermions::fill_kernels()
{
    ...
    if (kernelParameters->getFermact() == common::action::wilson) {
        ...
        if (kernelParameters->getUseEo() == false)
            throw Print_Error_Message("no kernels for CLOVER-discretization without even-odd preconditioning "
...
void hardware::code::Fermions::clover_eo_device(const hardware::buffers::Spinor* in,
                                                const hardware::buffers::Spinor* out, const hardware::buffers::SU3* gf,
                                                int evenodd, hmc_float kappa, hmc_float csw) const
{
    // get kappa
    hmc_float kappa_tmp;
    if (kappa == ARG_DEF)
...
void hardware::code::Fermions::clover_eo_inverse_device(const hardware::buffers::Spinor* in,
                                                        const hardware::buffers::Spinor* out,
                                                        const hardware::buffers::SU3* gf, int evenodd, hmc_float kappa,
                                                        hmc_float csw) const
{
    // get kappa
    hmc_float kappa_tmp;
